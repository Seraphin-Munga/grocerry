    ...
    function($http) {
      ...
      this.addOneProduct = function(product) {
        ...
        if (product.outOfStock === 0) {
          ...
          product.total =
            '<i class="fa fa-usd" > ' +
            parseFloat(product.price) * parseInt(product.purchaseQuantity) +
            "</i>";
        } else {
      ...
      this.getProducts = function(callback) {
        $http.get("/admin/panel/products").success(
          function(products) {
...
(function(angular) {
  ...
  var app = angular.module("ionicShop.directives", [
    "ionic",
    "ionicShop.services"
  ]);

  app.directive("ionCart", [
    "Products",
    "$templateCache",
    function(Products, $templateCache) {
  ...
  app.directive("ionCheckoutFooter", [
    "$compile",
    "Products",
    "stripeCheckout",
    "CheckoutValidation",
      ...
      var link = function(scope, element, attr) {
        ...
        element.on("click", function() {
          if (CheckoutValidation.checkAll(scope.checkout)) {
            ...
            var ionPurchaseSpan = document.getElementsByTagName(
              "ion-checkout"
            )[0].children[0];
            angular
              .element(ionPurchaseSpan)
              .html("You have invalid fields:")
              .css({ color: "#ED303C", opacity: 1 });
          }
        });
