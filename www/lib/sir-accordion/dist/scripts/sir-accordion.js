    ...
    link: function(scope,element) {
      ...
      */
      var itemRegen = function(collection, parentIndex, currentIndex, level) {
		...
		if(parentIndex<1){
			if(collection[currentIndex].subcategory!=null){
			...
			if(scope.config.menuLink!=''){
				var catLink = scope.config.menuLink+"/"+collection[currentIndex].id;
        ...
        if (angular.isArray(collection[currentIndex].subcategory) && collection[currentIndex].subcategory.length){
          item = item + itemRegen(collection[currentIndex].subcategory, uniqueIndex, 0, level + 1);
